...
def _publication_export_dir(dataset_key: str, output_dir: Path) -> Path:
    return output_dir / f"scraped_publications_{dataset_key}_parquet"


def _write_partitioned_parquet(
    df: pd.DataFrame,
    output_dir: Path,
    partition_col: str = "publication_year",
    chunk_size: int = 25_000,
) -> None:
    """Write a dataframe as small zstd-compressed parquet parts.

    The dashboard reads from data/cache/*.parquet. The files under
    data/outputs/ are optional downloadable/export artefacts, so we split them
    into small parquet parts to avoid GitHub's single-file size limit.
    """
    ...
    if df.empty:
        df.to_parquet(output_dir / "part-00000.parquet", index=False, compression="zstd")
        return

    tmp = df.copy()
    if partition_col not in tmp.columns:
        tmp[partition_col] = "unknown"
    tmp[partition_col] = tmp[partition_col].astype("string").fillna("unknown")

    for value, group in tmp.groupby(partition_col, dropna=False, sort=True):
        safe_value = str(value).replace("/", "_").replace("\\", "_").replace(" ", "_")
        part_dir = output_dir / f"{partition_col}={safe_value}"
        part_dir.mkdir(parents=True, exist_ok=True)
        group = group.reset_index(drop=True)
        for i, start in enumerate(range(0, len(group), chunk_size)):
            chunk = group.iloc[start : start + chunk_size]
            chunk.to_parquet(part_dir / f"part-{i:05d}.parquet", index=False, compression="zstd")
...
def save_tables(
    ...
    # Optional publication-level export for downstream analysis.
    # Keep this parquet-only and split into parts so generated artefacts stay
    # well below GitHub's single-file limit. The Streamlit app itself reads
    # from data/cache/*.parquet, not from data/outputs/.
    for stale in export_output_dir.glob(f"scraped_publications_{dataset_key}*.csv"):
        stale.unlink(missing_ok=True)
    stale_single_parquet = export_output_dir / f"scraped_publications_{dataset_key}.parquet"
    stale_single_parquet.unlink(missing_ok=True)

    pub_export_dir = _publication_export_dir(dataset_key, export_output_dir)
    if not tables["roster_works"].empty:
        ...
        export = tables["roster_works"].copy()
    _write_partitioned_parquet(export, pub_export_dir)
    paths["scraped_publications_parquet_dir"] = pub_export_dir
...
def build_cache_from_staff_csv(
    ...
    _log(progress, "Saved generated parquet cache and optional parquet publication export.")
